...
from .utils.coarse_matching import CoarseMatching, zeroshot_coarse_matching
...
class LoFTR(nn.Module):
    def __init__(self, config):
        ...
        self.backbone.encode.load_state_dict(torch.load('weights/resnet50-0676ba61.pth', map_location='cpu'))
    ...
    def forward(self, data):
        ...
        # match groundtruth images
        if data['gt'].sum():
            gt = data['gt']
            self.coarse_matching(feat_c0[gt], feat_c1[gt], data, mask_c0=mask_c0[gt], mask_c1=mask_c1[gt])

        # match zeroshot images
        if data['zs'].sum():
            zs = data['zs']
            zeroshot_coarse_matching(feat_c0[zs], feat_c1[zs], data, mask_c0[zs], mask_c1[zs], temperature=self.coarse_matching.temperature, sample_num=self.config['match_coarse']['train_zeroshot_num'])
