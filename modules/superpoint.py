...
from modules.backbone.base_model import BaseModel
from modules.submodules.misc import pad_and_stack
    ...
    def _forward(self, data):
        ...
        if self.conf.sparse_outputs:
            ...
            if Nz:
                ...
                pseudo_keypoints = [pseudo_labels[b_ids == i] for i in range(Nz)]
                pseudo_keypoints = pad_and_stack(
                    pseudo_keypoints, max_kps, -2, mode="random_c",
                    bounds=(0, data.get("image_size", torch.tensor(image.shape[-2:])).min().item()),
                )
                keypoints[zs] = pseudo_keypoints
